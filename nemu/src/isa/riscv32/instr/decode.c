...
static def_DHelper(B) {
  ...
  decode_op_i(s, id_dest, s->isa.instr.b.simm12 << 12 | s->isa.instr.b.imm11 << 11 
  | s->isa.instr.b.imm10_5 << 5 | s->isa.instr.b.imm4_1 << 1, false);
}
...
static def_DHelper(J) {
  ...
  decode_op_i(s, id_src1, s->isa.instr.j.simm20 << 20 |
   s->isa.instr.j.imm19_12 << 12 | s->isa.instr.j.imm11 << 11 
   | s->isa.instr.j.imm10_1 << 1, false);
}
...
def_THelper(main) {
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01100 11", R     , reg); //register
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00000 11", I     , load);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , iarithm); //immediate arithmetic
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11001 11", I     , jalr);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01000 11", S     , store);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11000 11", B     , conb); // conditional branch
